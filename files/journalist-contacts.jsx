...
// Kokoro voice assignments — British voices where available, matched to character
const contacts = [
  {
...
async function getKokoro(onProgress) {
  ...
  try {
    ...
    if (!hasWebGPU) onProgress?.("WebGPU unavailable — using CPU fallback (slower)…");
...
function splitSentences(text) {
  // Split on sentence boundaries, keep segments reasonably sized
...
async function speakKokoro(text, voice, onStart, onEnd, onProgress, setCurrentAudio) {
  ...
  const speakLimit = Math.min(sentences.length, 2); // speak at most 2 sentences
...
function speakBrowser(text, onStart, onEnd) {
  ...
  const doSpeak = () => {
    ...
    utter.voice = all.find(v => v.lang === "en-GB") || all.find(v => v.lang.startsWith("en")) || all[0];
...
async function transcribeGroq(audioBlob, filename = "audio.webm") {
  const form = new FormData();
  form.append("file", audioBlob, filename);
  form.append("model", "whisper-large-v3");
  form.append("language", "en");
  const res = await fetch("/api/chat?type=transcription", {
    method: "POST",
    body: form
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(`Groq ${res.status}: ${err?.error?.message || "transcription failed"}`);
  }
...
function createRingTone() {
  ...
  // UK double-ring: 400ms on, 200ms off, 400ms on, 2000ms off
  ...
  const playRing = (startTime) => {
    ...
    for (let i = 0; i < pattern.length; i++) {
      if (i % 2 === 0) {
        // Tone on — two frequencies for classic BT ring sound
...
function KeyField({ label, value, onChange, placeholder, hint }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ color: "rgba(255,255,255,0.55)", fontSize: 11, fontWeight: 600, letterSpacing: 0.8, marginBottom: 6, textTransform: "uppercase" }}>{label}</div>
      <div style={{ display: "flex", gap: 8 }}>
        <input type={show ? "text" : "password"} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
          style={{ flex: 1, background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, padding: "10px 12px", color: "#fff", fontSize: 13, caretColor: "#34C759", outline: "none" }} />
        <button onClick={() => setShow(s => !s)} style={{ background: "rgba(255,255,255,0.07)", border: "none", borderRadius: 10, width: 40, cursor: "pointer", color: "rgba(255,255,255,0.5)", fontSize: 13 }}>
          {show ? "🙈" : "👁"}
        </button>
      </div>
      {hint && <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 11, marginTop: 5, lineHeight: 1.5 }}>{hint}</div>}
    </div>
  );
}
...
export default function App() {
  ...
  const [kokoroReady, setKokoroReady] = useState(isMobile); // mobile skips Kokoro
  ...
  const canSend = callStatus === "connected" && !loading && !speaking && !listening && !transcribing;
...
Expertise: ${contact.topics}. If asked outside your expertise, politely deflect.`;
  ...
  // ── Voice recording ──
  const startListening = useCallback(async (contact) => {
    if (!canSend) return;
    stopAudio();
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          const text = await transcribeGroq(blob, `audio.${ext}`);
          setStatusLabel("");
          setTranscribing(false);
          if (text) sendMessage(text, contact);
        } catch (e) {
          setStatusLabel(`Transcription failed: ${e.message}`);
          setTranscribing(false);
          setTimeout(() => setStatusLabel(""), 4000);
        }
      };
      recorder.start();
      setListening(true);
      ...
      setStatusLabel("Mic access denied");
  ...
  const endCall = () => {
    ...
    setSpeaking(false); setListening(false); setTranscribing(false); setStatusLabel("");
    setCallStatus("ended");
    setTimeout(() => { setScreen("contacts"); setCallStatus("idle"); setActiveContact(null); setTranscript([]); setInput(""); }, 900);
  };

  // Unified speak — Kokoro on desktop, browser TTS on mobile
  ...
  useEffect(() => {
    if (isMobile) return;
    setKokoroStatus("Downloading voice model… (first visit only)");
    getKokoro((msg) => setKokoroStatus(msg))
      .then(() => { setKokoroReady(true); setKokoroStatus(""); })
      .catch(() => { setKokoroStatus("Voice model unavailable — browser fallback will be used"); setKokoroReady(true); });
  }, []);
  ...
  // ─────────────────────────────────────────────────────────────
  return (
    <div style={{ fontFamily: "'SF Pro Display',-apple-system,'Helvetica Neue',sans-serif", background: "#060606", minHeight: "100vh", display: "flex", alignItems: isMobile ? "stretch" : "center", justifyContent: "center", padding: isMobile ? 0 : 20 }}>
      <style>{`
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", background: `linear-gradient(170deg,#0d0d1a 0%,#111827 50%,${activeContact.color}18 100%)`, animation: "fadeIn 0.35s ease", minHeight: 0 }}>
            <div style={{ padding: isMobile ? "calc(env(safe-area-inset-top, 44px) + 8px) 24px 0" : "10px 24px 0", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, flexShrink: 0 }}>
              ...
              <div style={{ minHeight: 28, display: "flex", alignItems: "center", justifyContent: "center", width: "100%" }}>
                ...
                {(transcribing||loading||statusLabel)&&!speaking&&!listening&&(
            ...
            {callStatus==="connected"&&(
              <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column", padding: "0 14px", minHeight: 0 }}>
                <div ref={transcriptRef} style={{ flex: 1, overflowY: "auto", padding: "4px 0", display: "flex", flexDirection: "column", gap: 8 }}>
                  {transcript.length===0&&(
                    <div style={{ color: "rgba(255,255,255,0.22)", fontSize: 13, textAlign: "center", padding: "16px", lineHeight: 1.8 }}>
                      ...
                      <span style={{fontSize:11,opacity:0.7}}>Voices generated locally via Kokoro</span>
                    </div>
                  )}
                ...
                {/* Input row */}
                <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", padding: "10px 0 10px" }}>
